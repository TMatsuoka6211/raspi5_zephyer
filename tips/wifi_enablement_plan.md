...
### 課題 1: WSL上での Windows FAT パーティションマウント失敗
- **事象**: WSL で SD カードへバイナリ (`zephyr.bin`) をコピーする際、マウントポイントが `System Volume Information` フォルダ等の存在や Windows 側の排他制御によりロックされ、コピーがエラーになる、あるいは古いバイナリが書き込まれ続ける現象が発生。
- **解決策**: Windows 側の不要なディレクトリを手動でクリーンアップし、`sudo mount -t drvfs` で確実な同期オプションを伴う再マウント手順を適用して解決。

### 課題 2: MMU 仮想メモリ変換テーブルの枯渇 (`xlat tables low`)
...
### 課題 3: EL3 (Secure Mode) のピンアクセス制御による SError クラッシュ
...
### 課題 4: CMD8 のタイムアウト判定ミスマッチによるスタックのアボート
